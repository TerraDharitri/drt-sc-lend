import { BigIntValue, TokenPayment } from "@numbatnetwork/drtjs";
import { createAirdropService, FiveMinutesInMilliseconds, createDCDTInteractor, INetworkProvider, ITestSession, ITestUser, TestSession } from "@numbatnetwork/drtjs-snippets";
...
describe("lending snippet", async function () {
    ...
    it("Airdrop REWA", async function () {
        this.timeout(FiveMinutesInMilliseconds);
        
        let payment = TokenPayment.rewaFromAmount(0.1);
    ...
    it("Issue Pool Token ABC", async function () {
        ...
        let interactor = await createDCDTInteractor(session);
        await session.syncUsers([whale]);
    ...
    it("withdraw token XYZ", async function () {
        ...
        let paymentXYZ = TokenPayment.metaDcdtFromAmount(lendTokenXYZ, depositNonceXYZ, 7, tokenXYZ.decimals)
    ...
    it("borrow ABC token - collateral XYZ", async function () {
        ...
        let collateralPayment = TokenPayment.metaDcdtFromAmount(lendTokenXYZ, depositNonceXYZ, 5, tokenXYZ.decimals)
    ...
    it("repay ABC token - collateral XYZ", async function () {
        ...
        let paymentOne = TokenPayment.metaDcdtFromAmount(borrowedTokenABC, borrowedNonceABC, 3.21, tokenXYZ.decimals);
